...
DWORD WINAPI SnakeGame::handle_client(LPVOID lpParam) {
    SnakeTcp* tcp = (SnakeTcp*)lpParam;
    SnakeGame game;
    SOCKET client = tcp->ClientSocket;
...
int main() {
    ...
    while (1) {
        ...
        DWORD thread_id;
        HANDLE thread_handle = CreateThread(NULL, 0, game.handle_client, tcp, 0, &thread_id);
